...
function parseMeminfo(raw) {
  ...
  if (!available)
    available = kbToBytes(map.MemFree) + kbToBytes(map.Cached) + kbToBytes(map.Buffers);
...
function parseLscpu(raw) {
  ...
  if (flagText)
    flags = flagText.split(/\s+/).filter(function (f) {
      return !!f;
    });
  return {
...
function pickChipset(devices) {
  ...
  return {
    ...
    southbridge:
      host && south && south.name && south.name !== (host && host.name)
...
function isNpuDevice(row) {
  ...
  return (
    blob.indexOf("neural processing") !== -1 ||
    blob.indexOf("npu") !== -1 ||
    blob.indexOf("xdna") !== -1 ||
    blob.indexOf("tpu") !== -1 ||
    blob.indexOf("ai accelerator") !== -1 ||
    blob.indexOf("habana") !== -1 ||
    blob.indexOf("hailo") !== -1 ||
    blob.indexOf("coral") !== -1
  );
}
...
function chassisTypeName(code) {
  var n = intOrZero(code);
  if (!n && typeof code === "string" && code && !/^[0-9]+$/.test(code)) return cleanText(code, 40);
...
function normalize(raw) {
  ...
  out.cpu.flags = asArray(cpu.flags)
    .map(function (f) {
      return cleanText(f, 32);
    })
    .filter(function (f) {
      return !!f;
    });
  ...
  out.memory.modules = asArray(memory.modules)
    .map(normalizeModule)
    .filter(function (m) {
      return m.size > 0 || m.locator;
    });

  out.gpus = asArray(data.gpus)
    .map(normalizeGpu)
    .filter(function (g) {
      return g.name || g.pciId;
    });
  out.npus = asArray(data.npus)
    .map(function (item) {
      var row = asObject(item);
      return {
        name: cleanText(row.name, 160),
    ...
    .filter(function (n) {
      return n.name || n.pciId;
    });
  out.nics = asArray(data.nics)
    .map(normalizeNic)
    .filter(function (n) {
      return n.iface || n.name;
    });
  out.audio = asArray(data.audio)
    .map(normalizeAudio)
    .filter(function (a) {
      return a.name;
    });
  out.usb = asArray(data.usb)
    .map(normalizeUsb)
    .filter(function (u) {
      return u.name || u.product;
    });
  out.batteries = asArray(data.batteries)
    .map(normalizeBattery)
    .filter(function (b) {
      return b.name || b.capacity;
    });
  out.thermals = asArray(data.thermals)
    .map(normalizeThermal)
    .filter(function (t) {
...
function cpuSummary(cpu) {
  ...
  if (row.threads && row.threads !== row.cores) topo.push(row.threads + " threads");
...
function biosSummary(bios) {
  ...
  return sentence([joinParts([row.vendor, row.version], " "), row.date, row.uefi ? "UEFI" : ""]);
}
...
function parseGpuIdentity(lspciText) {
  ...
  return parseHwIdentity(
    String(gpus[0].name || "").replace(/\s*\[[0-9a-fA-F]{4}:[0-9a-fA-F]{4}\].*$/, ""),
  );
}
...
function parseNpuIdentity(lspciText) {
  ...
  return parseHwIdentity(
    String(npus[0].name || "").replace(/\s*\[[0-9a-fA-F]{4}:[0-9a-fA-F]{4}\].*$/, ""),
  );
}
