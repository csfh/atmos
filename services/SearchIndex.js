...
const HUBS = [
  { id: 'appearance', title: 'Appearance', description: 'Theme, wallpaper, fonts, and how the desktop looks.', keywords: ['theme', 'background', 'wallpaper', 'font', 'text', 'size', 'palette', 'nightlight'] },
  { id: 'display', title: 'Displays', description: 'Monitors, scale, and brightness.', keywords: ['monitor', 'scale', 'brightness'] },
  { id: 'hardware', title: 'Hardware', description: 'CPU, GPU, memory, and firmware.', keywords: ['cpu', 'gpu', 'memory'] },
  { id: 'windows', title: 'Windows', description: 'Gaps, borders, bindings, and window rules.', keywords: ['gaps', 'bind', 'window'] },
  { id: 'input', title: 'Input', description: 'Pointer, keyboard, and gestures.', keywords: ['mouse', 'keyboard'] },
  { id: 'accessibility', title: 'Accessibility', description: 'Motion, text size, and assistive tools.', keywords: ['a11y', 'motion'] },
  { id: 'sound', title: 'Sound', description: 'Volume, sinks, and sources.', keywords: ['audio', 'volume'] },
  { id: 'capture', title: 'Capture', description: 'Screenshots, recordings, and OCR.', keywords: ['screenshot', 'record'] },
  { id: 'disks', title: 'Disks', description: 'Drives, snapshots, and swap.', keywords: ['drive', 'snapper', 'swap'] },
  { id: 'bar', title: 'Bar', description: 'Position, clock, and widgets.', keywords: ['bar', 'clock', 'tray'] },
  { id: 'notifications', title: 'Notifications', description: 'Do not disturb and reminders.', keywords: ['dnd', 'reminder'] },
  { id: 'defaults', title: 'Defaults', description: 'Browser, terminal, editor, and MIME.', keywords: ['browser', 'terminal'] },
  { id: 'applications', title: 'Applications', description: 'Desktop, TUI, and web launchers.', keywords: ['app', 'launcher'] },
  { id: 'software', title: 'Software', description: 'Packages and extras.', keywords: ['install', 'package'] },
  { id: 'network', title: 'Network', description: 'Wi-Fi, Bluetooth, DNS, and speed test.', keywords: ['wifi', 'bluetooth', 'dns'] },
  { id: 'power', title: 'Power', description: 'Profiles and battery.', keywords: ['battery', 'profile'] },
  { id: 'idle', title: 'Idle and lock', description: 'Screensaver, lock, and lid.', keywords: ['lock', 'screensaver'] },
  { id: 'security', title: 'Security', description: 'Fingerprint, SSH, and sudo.', keywords: ['ssh', 'fingerprint'] },
  { id: 'hooks', title: 'Hooks', description: 'Theme-set and other scripts.', keywords: ['hook', 'script'] },
  { id: 'system', title: 'System', description: 'Host, locale, updates, and about.', keywords: ['hostname', 'locale', 'update'] }
]
...
function ingestRows(db, rows) {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    insert.run(
      ...
      rowHaystack({ label: row.label, description: row.description, hint: row.hint, detail: row.detail, keywords: keywords })
    )
  }
}
...
function likeNeedle(query) {
  return '%' + String(query || '').replace(/\\/g, '\\\\').replace(/%/g, '\\%').replace(/_/g, '\\_') + '%'
}
...
function queryRows(db, query) {
  ...
  return db.prepare(
    "SELECT * FROM rows WHERE haystack LIKE ? ESCAPE '\\' ORDER BY hub, label"
  ).all(likeNeedle(q)).map(mapRow)
}
...
function slug(text) {
  return String(text || '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || 'row'
}
...
function rowsFromQml(src, hub) {
  ...
  for (let k = 0; k < kinds.length; k++) {
    ...
    for (let i = 0; i < blocks.length; i++) {
      ...
      const label = kinds[k] === 'PrefsGroup' ? stringProp(block, 'title') : stringProp(block, 'label')
      if (!label) continue
      out.push({
...
function main(argv) {
  ...
  if (args.cmd !== 'query' && args.cmd !== 'state') {
    process.stderr.write('usage: SearchIndex.js query <text> [--rows file] [--snapshot file] [--root dir]\n')
